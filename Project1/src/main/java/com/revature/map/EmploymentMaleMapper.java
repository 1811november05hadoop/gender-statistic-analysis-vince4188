	...
	protected void map(LongWritable key, Text value, Context context)
		...
		if (line.toLowerCase().contains("unemployment") && line.toLowerCase().contains(" male ") && line.toLowerCase().contains("ilo")){
			...
			if (splits.length > 4){
				int count = 0;
				double prevVal = 0.0;
				//just like before, used to find difference starting from 2000-2016
				...
				if (count == 0){
					//the -1.0 will be used to stop the reducer from processing the data should it recieve one
